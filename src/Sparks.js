...
const r = () => Math.max(0.15, Math.random())
...
export default function Sparks({ mouse, count, colors, radius = 10 }) {
  const lines = useMemo(
    () =>
      new Array(count).fill().map((_, index) => {
        ...
        const points = new Array(60).fill().map((_, index) => {
        ...
        return {
          ...
          width: Math.max(0.1, (0.12 * index) / 10),
  ...
  return (
    <group ref={ref}>
      <group position={[-radius * 2, -radius, -10]} scale={[1, 1.25, 1]}>
